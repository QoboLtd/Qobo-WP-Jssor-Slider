...
<div id="slider_container_<?php echo $slider_id; ?>"
     style="position: relative; width:<?php echo $sliderW; ?>px; height:<?php echo $sliderH; ?>px;"
     class="jssor-slider">
    <!-- Slides Container -->
    <div u="slides"
         style="cursor: move; position: absolute; left: 0px; top: 0px; width:<?php echo $sliderW; ?>px; height:<?php echo $sliderH; ?>px; overflow: hidden;">
        <!-- Slide -->
        <?php for ($flag = 0; $flag < count($slides); $flag++) : ?>
            <div>
                ...
                <?php if ($slides[$flag]->title) : ?>
                    <div u="caption" t="<?php echo $result2[$slides[$flag]->caption_in]; ?>"
                         t2="<?php echo $result2[$slides[$flag]->caption_out]; ?>"
                         style="position: absolute; top: 20px; left: 20px; height: 30px; color: #ffffff; font-size: 20px; line-height: 30px;"><?php echo html_entity_decode($slides[$flag]->title); ?></div>
                <?php endif;
                if ($slides[$flag]->description) :
                    ?>
                    <div u="caption" t="<?php echo $result2[$slides[$flag]->description_in]; ?>"
                         t2="<?php echo $result2[$slides[$flag]->description_out]; ?>"
                         style="position: absolute; top: <?php echo $descH; ?>px; left: 0px;width: 800px; height: 50px;">
                        <div
                            style="position: absolute; top: 0px; left: 0px; width: <?php echo $sliderW; ?>px; height: 50px;background-color: Black; opacity: 0.5; filter: alpha(opacity=50);"></div>
                        <div
                            style="position: absolute; top: 0px; left: 0px; width: <?php echo $sliderW; ?>px; height: 30px;color: White; font-size: 16px; font-weight: bold; line-height: 30px; text-align: center;">
                            <?php if ($slides[$flag]->url) : ?>
                                <a style="text-decoration:none;" target="_blank"
                                   href="<?php echo html_entity_decode($slides[$flag]->url); ?>"><?php echo html_entity_decode($slides[$flag]->description); ?></a>
                            <?php else : ?>
    ...
    <?php if ($use_arrows) : ?>
        ...
        <!-- Arrow Left -->
        <span u="arrowleft" class="<?php echo $arrowL; ?>"
              style="width:<?php echo $arrowWidth; ?>px; height:<?php echo $arrowHeight; ?>px; top: 45%; left: <?php echo $arrowBorder ? '0' : '8' ?>px;"></span>
        <!-- Arrow Right -->
        <span u="arrowright" class="<?php echo $arrowR; ?>"
              style="width:<?php echo $arrowWidth; ?>px; height:<?php echo $arrowHeight; ?>px; top: 45%; right: <?php echo $arrowBorder ? '0' : '8' ?>px"></span>
    <?php endif; ?>
